...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add keyboard event listeners for virtual keyboard
    document.addEventListener('keydown', handleKeyDown);
    document.addEventListener('keyup', handleKeyUp);
});

// Function to handle key down events for the virtual keyboard
function handleKeyDown(event) {
    ...
    if (key === ' ') {
        ...
        if (spaceKey) activateKey(spaceKey);
    } else {
        // For all other keys
        const keyElements = document.querySelectorAll(`.key[data-key="${key}"], .key[data-key="${key.toLowerCase()}"]`);
        keyElements.forEach(keyElement => activateKey(keyElement));
    }
}

// Function to handle key up events for the virtual keyboard
function handleKeyUp(event) {
    ...
    if (key === ' ') {
        // For Space key
        const spaceKey = document.querySelector('.key[data-key=" "]');
        if (spaceKey) deactivateKey(spaceKey);
    } else {
        // For all other keys
        const keyElements = document.querySelectorAll(`.key[data-key="${key}"], .key[data-key="${key.toLowerCase()}"]`);
        keyElements.forEach(keyElement => deactivateKey(keyElement));
    }
}

// Function to activate a key on the virtual keyboard
function activateKey(keyElement) {
    if (keyElement) {
        keyElement.classList.add('active');
        
        // Add rainbow glow effect
        const keyContent = keyElement.querySelector('.key-content');
        if (keyContent) {
            keyContent.style.color = 'var(--primary)';
        }
    }
}

// Function to deactivate a key on the virtual keyboard
function deactivateKey(keyElement) {
    if (keyElement) {
        keyElement.classList.remove('active');
        
        // Remove rainbow glow effect
        const keyContent = keyElement.querySelector('.key-content');
        if (keyContent) {
            keyContent.style.color = '';
        }
    }
}
